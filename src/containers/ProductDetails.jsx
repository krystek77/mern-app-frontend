...
import { addLastWatched } from "../api/watched";
...
export async function loader({ params, request }) {
  ...
  try {
    ...
    const responseSimilarProducts = await api.getProductsByTags(queryTags, model);
...
export default function ProductDetails() {
  const { product, queryTags, queryTitle, similarProducts, priceList, avarageExchangeRate, user, categoryName } =
    useLoaderData();
  ...
  return (
    <>
      ...
      <Header
        ref={ref}
        bgGradient={
          product.coin ? "bg-gradient-radial-circle-from-cc-accent" : "bg-gradient-radial-circle-from-cc-primary"
        }
        title={
          product.coin
      ...
      <main className='px-8 pb-8 pt-16 mx-auto relative'>
        ...
        <div className='grid xl:grid-cols-1-300 xl:gap-x-6'>
          <article
            ...
            <div className={product.wide ? "col-span-1" : "lg:col-span-2"}>
              ...
              <div className='specyfication w-full mb-4'>
                ...
                <div className='table-head grid grid-cols-3 items-center border bg-primary text-white border-primary-dark rounded-tr rounded-tl'>
                  <span className='pl-4 py-2 justify-self-start font-bold'>nazwa</span>
                  <span className='py-2 justify-self-center font-bold'>jednostka</span>
                  <span className='pr-4 py-2 justify-self-end font-bold'>wartość</span>
                </div>
                <div className='[&>*:nth-child(even)]:bg-accent-light'>
                  {product.parameters.map((parameter) => (
                    <div key={parameter._id} className='grid grid-cols-3 items-center border-b border-accent-dark '>
                      <span className='pl-4 py-2 font-medium'>{parameter.name}</span>
                      <span className='py-2 font-medium justify-self-center'>{parameter.unit}</span>
                      <span className='pr-4 py-2 font-medium justify-self-end'>{parameter.value}</span>
                    </div>
                  ))}
                </div>
              </div>
                        ...
                        <div
                          ...
                          <div className='tab-content border mb-2 border-accent-dark rounded-bl rounded-br bg-slate-100 py-2 px-4 grid grid-cols-2 gap-x-4'>
                            <div className='tab-image justify-self-center self-center max-w-[300px] max-h-[200px] '>
                              <img className='object-fit w-full h-full' src={control.image} alt={control.name} />
                            </div>
                            {control.list.length ? (
            ...
            {/** pricelist */}
            {user ? (
              <div className={product.wide ? "max-w-2xl col-span-1 relative" : "max-w-2xl lg:col-span-2 relative"}>
                <PricesTable
                  priceList={priceList}
              ...
              <Form className='flex items-center justify-start flex-wrap'>
                ...
                <div className='w-full action-buttons flex justify-center mt-2'>
                  <button
                    ...
                    <span className='self-center justify-self-center lowercase'>Szukaj</span>
                  </button>
                  <button
                    className='grid grid-cols-2 items-center border border-slate-800 px-2 py-1 mx-2 rounded text-xs hover:shadow-lg'
                    type='button'
                    ...
                    <span className='self-center justify-self-center lowercase'>Anuluj</span>
                  </button>
                </div>
              </Form>
            ) : (
              <div className='col-span-5 text-lg font-light text-slate-500'>Produkt nie ma zdefiniowanycyh tagów</div>
            )}
          </div>
